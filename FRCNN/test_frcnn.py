...
for idx, img_name in enumerate(sorted(os.listdir(img_path))):
    ...
    for key in bboxes:
        ...
        total = "Caminhão com {} eixo(s)".format(len(range(new_boxes.shape[0])))
        font_path = os.path.join(os.path.dirname(__file__), "font/FiraMono-Medium.otf")
        font = ImageFont.truetype(
            font=font_path, size=np.floor(3e-2 * img2.size[1] + 0.5).astype("int32")
        )
        thickness = (img2.size[0] + img2.size[1]) // 300  
        ...
        for jk in range(new_boxes.shape[0]):
            ...
            if real_y1 - label_size[1] >= 0:
                text_origin = np.array([real_x1, real_y1 - label_size[1]])
            else:
                text_origin = np.array([real_x1, real_y2])
    ...
    img2.save(os.path.join(save_path, os.path.basename(img_name)))
    print(all_dets)
...
print(
		"Processed {} images in {:.1f}sec - {:.1f}FPS".format(
			...
			end_t - start_t,
			len(os.listdir(img_path)) / (end_t - start_t),
)
)
